...
const useMarvelService = () => {
  ...
  const _apiKey = "apikey=xco78vi86djfg3icv678trxio234mk423k";

  const _charBaseOffset = 210;
  const _charLimit = 9;
  const _comicsBaseOffset = 0;
  const _comicsLimit = 8;

  const getAllCharacters = async (offset = _charBaseOffset) => {
    const res = await request(
      `${_apiBase}characters?limit=${_charLimit}&offset=${offset}&${_apiKey}`
    );
